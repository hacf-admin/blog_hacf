...
- ***esp_eau_debit_eau_froide*** : mesure le **débit instantané**. Si au bout de 4s il n'y a plus d'impulsion, le débit se remet à 0. C'est un choix : les débits faibles seront mesurés en effectuant des différences de mesures sur de longues périodes, et donc ne sont pas traités avec l'entité débit. Un `filter` permet de multiplier la valeur par 4 pour obtenir des litres/mn (pour mémoire, nous avons 1 impulsion tous les 0.25l).
