---
draft: false
toc: true
visibleInCMS: true
date: 23-03-12
lastmod: 2022-09-28 23:48
socialshare: true
title: Association "Communauté Francophone autour de Home Assistant"
type: page
aliases:
  - /association
  - /about
description: Venez découvrir l'association communauté francophone autour de Home
  Assistant, son but et ses objectifs.
tags:
  - association
  - statuts
  - reglement
---
{{< btnAsso >}}

> L'association **HACF** est née de l'envie de réunir une **Communauté Francophone autour de Home ssistant**

## Objectifs.

Le but de l'association est de rendre **visible** et **accessible**, la solution domotique **gratuite** et **open source** Home Assistant, mais aussi de réunir le maximum d’utilisateurs en un seul écosystème à travers de nombreux supports : wiki (actualité et articles), forum d'échange, chat, vidéos ….

Plusieurs systèmes domotiques ont été testés par les fondateurs de HACF : Domoticz, Jeedom, OpenHab et bien d’autres. Finalement, nous souhaitons apporter un peu de visibilité à ce que nous **considérons** comme **LA solution domotique** qui surpasse largement ses concurrents, en termes de **nouveautés**, de **changements**, d’**intégrations** et de **croissance**.

Home Assistant est **open-source**, supporté et développé par une des plus importantes communautés **mondiale de développeurs** dans le domaine de la domotique. La totalité des **modules**, **add-ons**, **applications mobiles** sont gratuits.

Enfin, la solution est fédérée par une société qui garantit sa stabilité et sa cohérence, **Nabu Casa**. Elle propose un **service d’abonnement** totalement optionnel permettant soutenir la solution, mais aussi offrir à l’inscrit des **intégrations clés en main**, en particulier un **accès externe sécurisé** (là où l’utilisateur sans abonnement devrait suivre un des articles présents ici).

## Historique.

**Idée / Réflexion** *avril 2020*

* Naissance dans la tête de six personnes suite au manque observé d’une véritable communauté francophone autour de Home Assistant.

**Ouverture** *juillet 2020*

* HACF devient accessible à tous les francophones désirant partager, échanger ou apprendre sur la solution Home Assistant.

**Union** *février 2021*

* Union avec le groupe Facebook [Home Assistant France](https://groupe-facebook.hacf.fr)

**Association** *Juin 2021*

* Création de l’association [Communauté Francophone autour de Home Assistant](/association) dans un souci de transparence et de résilience.

**Union** *aout 2022*

* Union avec le forum home-assistant.lu dans le but de concentrer nos forces de partage et d'entraide en un seul lieu.

## L'association.

L'**association** s'est avérée comme un choix évident dès le début de l'aventure HACF, car elle permet de **pérenniser** la communauté contre tout départ d'un administrateur ou d'un fondateur.

Certes nous apportons la **transparence** la plus complète dans un sujet dédié sur le forum. Mais nous avons eu conscience que, malgré cela, nous devions créer une **entité morale** à cette communauté à travers une **association loi 1901**.

Passer par une association permet plusieurs choses :

* **Réunir** en tant que membres des passionnés de domotique et de Home Assistant,
* Avoir une **entité juridique**,
* Bénéficier d’un **compte bancaire** au nom de l’association ainsi qu’un suivi de trésorerie,
* Pouvoir organiser des **événements** ou des **rencontres**,
* Permet de participer à des événements,
* Avoir l’assurance d’une **gestion transparente** pour les membres et les donateurs,
* **Financer** les coûts engendrés par la communauté,
* **Pérenniser** notre communauté.

Depuis le 18 mai 2021 l’association **« Communauté Francophone autour de Home Assistant »** est officiellement créée et nous en sommes très fiers. La parution au [journal officiel](https://www.journal-officiel.gouv.fr/associations/detail-annonce/associations_b/20210021/421) a eu lieu le 25 mai 2021.
Le bureau actuel est composé de :

* Anthony (@McFly) en Président et Clément (@Clemalex) en vice-président,
* Hugo (@TheD6Bel) en trésorier et Antoine (@Bilbon) en adjoint,
* Olivier (@Pozzi) en secrétaire et Jean-Baptiste (@jybi89) en adjoint.

Les statuts sont disponibles [ici](/statuts) et le règlement [ici](/reglement). L'**adhésion** est de seulement **2 euros**.

> Bien entendu, une association loi 1901 comme "Communauté Francophone autour de Home Assistant" ne permet pas de faire de bénéfices ou d'enrichir une quelconque personne.

{{< button "Adhérer" "https://adherer.hacf.fr" "fas fa-ribbon" >}}

## Financement.

Comme vous vous en doutez, la création, le maintien et l'évolution d'une communauté a un coût. Si celui-ci a été entièrement supporté par les membres fondateurs la première année, ils sont, depuis, complètement assumé par **votre adhésion à l'association, vos dons et liens affiliés**.

> **Le financement d’un projet est souvent la partie invisible de l’iceberg** pour les utilisateurs, mais malheureusement celle-ci est bien réel et peut être un vrai problème de fond pour l’équipe en charge du projet.

Au début de l'aventure, nous avions réfléchi à plusieurs sources de financement :

* ~~Publicités~~,
* Dons,
* Liens affiliés,
* Cotisation à travers une association.

### Publicités

Nous sommes des utilisateurs du web comme vous et nous ne sommes pas favorables à l'invasion de votre écran par de la publicité en tous genres, toutes les trois lignes d'article.
De plus nous ne contrôlons pas forcément le type de publicité diffusé.
Donc **solution NON retenue**.

### Cotisations et Dons

Les cotisations et les dons vous permettent d’être reconnaissants envers le travail de la communauté. Pour cela, nous avons retenu deux plateformes parmi plusieurs.

#### Paypal.me

Grâce à sa renommée et compatibilité, Paypal via son service [PayPal.me](https://paypal.hacf.fr/) nous a semblé être une plateforme pertinente.

* Pas de commissions, pour vous comme pour nous.
* Permet de payer directement les prestataires sans passer par un transfert qui peut, lui aussi, amputer de nouveau une partie des dons.

{{< button "Don" "https://paypal.hacf.fr" "fab fa-paypal">}}

#### Hello Asso

Suite à la création de l'association, nous avons cherché un service pour gérer les adhésions.
Il s'avère qu'Hello **Asso** propose d'autres services comme la possibilité de recevoir des dons.

* Pas de commissions, pour vous comme pour nous.
* Permet de faire un don pendant l'[adhésion](https://adhesion.hacf.fr/) ou alors [indifféremment](https://dons.hacf.fr/).

{{< button "Don" "https://dons.hacf.fr" "fas fa-hand-holding-dollar">}}

### Liens affiliés

Le principe d'un **lien affilié** est de reverser un pourcentage de votre panier si vous cliquez sur un lien à travers une plateforme compatible. Ils peuvent être utilisés pour n'importe quel type d'achat et pas seulement pour du matériel domotique.

> Important : cela n'a pas d'incidence sur votre prix.

Deux enseignes bien connues ont été retenues : **Amazon** et **AliExpress**. Elles ont chacune leurs avantages et inconvénients. Avec ces deux enseignes, nous balayons un large panel concernant vos attentes.

> Notre [plateforme discord](https://discord.hacf.fr) vous permet de générer les liens affiliés : allez sous le fil de discution [Liens-affiliés](https://discord.com/channels/706096417000652840/784046728282243102), coller le lien vers le produit (Amazon ou Aliexpress), et un bot vous renverra le lien affilié à utiliser.
>
> Il est possible en cliquant sur les liens ci-dessous d'accèder à l'`accueil affilié` des deux marchands.

#### Amazon

Ce géant de l'e-commerce international propose des produits qui arriveront rapidement avec un service client très satisfaisant, mais en contrepartie le/les produits seront plus chers.

{{< button "Liens Affiliés" "https://amazon.hacf.fr" "fab fa-amazon">}}

#### Aliexpress

Ce géant de l'e-commerce des marchés asiatiques vous permettra d’acheter à des prix défiants toute concurrence, mais la livraison sera plus longue entre 15 et 60 jours.
Cette enseigne est très intéressante dans les projets DIY avec des prix de pièces électroniques très bas.

> Il existe la livraison en 10 jours ouvrés pour des prix a peine plus chers.

{{< button "Liens Affiliés" "https://aliexpress.hacf.fr" "fa-brands fa-alipay">}}

### Les partenariats

Nous espérons pouvoir à terme mettre en place des partenariats avec des marques ou des enseignes permettant de présenter du matériel pouvant être intégré à Home Assistant, mais aussi proposer des bons de réductions à nos adhérents.

Si le matériel est donné à la communauté, alors il sera mis dans un jeu concours pour les membres de la communauté au cours de l'année.

> Si vous êtes une enseigne ou un entreprise et êtes intéressés à nous envoyer, que ce soit en don ou en prêt, du matériel à tester, alors [contactez-nous](https://contact.hacf.fr).

## Participer, Contribuer, Adhérer.

Voici les trois mots à retenir pour une communauté grandissante et pleine de volonté.

### Participer.

C'est grâce à vos participations que nos médias se remplissent et attirent de plus en plus de monde.
Les trois principaux médias sont le [Forum](https://forum.hacf.fr), le [groupe Facebook](https://groupe-facebook.hacf.fr/) et le [Discord](https://discord.hacf.fr/). Nous avons d'autres médias en préparation, et recherchons toujours des volontaires pour les animer et les abonder en contenu.

### Contribuer.

Contribuer financièrement ?? Et non, pas seulement et de loin. La **contribution** peut être en **temps**, en **énergie** et en **idées**.
Tout ce qui peut faire avancer la communauté est bienvenue : que cela soit pour aider à modérer, à animer nos réseaux sociaux (par exemple), **partager** des news liées à la domotique ou à Home Assistant, tenir à jour les tutoriels (oui Home Assistant évolue trop vite) et bien d'**autres choses**. Alors n’hésitez pas, contactez-nous[](https://contact.hacf.fr) et proposez vos idées ou votre candidature.

> Pour le reste, nous sommes tous concernés. A nous de faire ce que vous voulez de cette communauté. Alors n’hésitez pas, **il n’y a pas de bonne volonté inutile**.

D’autres choses arriveront encore avec le temps et en fonction de vos suggestions et contributions.

Il y a encore beaucoup de choses à dire et/ou à faire sur le fonctionnement et nous comptons sur vous.

{{< btnAsso >}}